...
// ============ FETCH MESSAGES BY USER ID (Full History) ============
export const fetchMessagesByUserId = createAsyncThunk(
  "chat/fetchMessagesByUserId",
  async (userId: string, { rejectWithValue }) => {
    try {
      // Calls the /api/chat/messages/user/${userId} endpoint
      const response = await chatAPI.fetchMessagesByUserId(userId);

      return response.data;
    } catch (error: any) {
      return rejectWithValue(
        error.response?.data || "Failed to fetch user messages",
      );
    }
  },
);
...
const chatSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // --- FETCH MESSAGES (By Application ID) ---
      .addCase(fetchMessages.pending, (state) => {
        state.loading = true;
      })
      // inside extraReducers for fetchMessages.fulfilled:
      .addCase(fetchMessages.fulfilled, (state, action) => {
        state.loading = false;
        // Ensure we always extract an array, even if the API returns { messages: [...] }
      ...
      // --- FETCH MESSAGES BY USER ID (The new one) ---
      ...
      .addCase(fetchMessagesByUserId.fulfilled, (state, action) => {
        ...
        // Use the same extraction logic to keep things consistent
        const incomingMessages = Array.isArray(action.payload)
          ? action.payload
          : action.payload?.messages || [];

        // Update the state if the content has changed
        if (
          JSON.stringify(state.messages) !== JSON.stringify(incomingMessages)
        ) {
          state.messages = incomingMessages;
      ...
      .addCase(fetchMessagesByUserId.rejected, (state, action) => {
        state.loading = false;
        state.error =
          (action.payload as any)?.message || "Failed to fetch user history";
      })
