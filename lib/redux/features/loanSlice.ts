...
export const calculateLoan = createAsyncThunk(
	"loans/calculateLoan",
	async (
		{ amount, months, rate }: { amount: number; months: number; rate: number },
		{ rejectWithValue },
	) => {
		try {
			const response = await loanAPI.getLoanCalculator(amount, months, rate);
			return response.data;
		} catch (error: any) {
			return rejectWithValue(
...
const loanSlice = createSlice({
	...
	extraReducers: (builder) => {
		...
		// Calculate Loan
		builder.addCase(calculateLoan.pending, (state) => {
			state.isLoading = true;
			state.error = null;
		});
